...
# Global model references (lazy loading)
_dam2 = None
_genstereo_nvs = None
_fusion_model = None
...
def get_sd_settings(fast_mode=False):
    """Get SD version settings based on fast_mode."""
    if fast_mode:
        return "v1.5", 512, 'genstereo-v1.5'
    else:
        return "v2.1", 768, 'genstereo-v2.1'

def load_models(fast_mode=False):
    """Load all models lazily."""
    global _dam2, _genstereo_nvs, _fusion_model, _current_sd_version, _IMAGE_SIZE, _CHECKPOINT_NAME

    sd_version, image_size, checkpoint_name = get_sd_settings(fast_mode)

    # Skip if already loaded with same settings
    if _current_sd_version == sd_version and _dam2 is not None:
        return image_size

    print(f"Loading models (SD {sd_version}, IMAGE_SIZE={image_size})...")
    ...
    _dam2 = DepthAnythingV2(**model_configs[encoder])
    ...
    _dam2.load_state_dict(torch.load(dam2_checkpoint, map_location='cpu'))
    _dam2 = _dam2.to(DEVICE).eval()
...
def infer_depth_dam2(image_np: np.ndarray) -> torch.Tensor:
    ...
    image_pil = crop(image_pil).resize((_IMAGE_SIZE, _IMAGE_SIZE), Image.BILINEAR)
    image_bgr = cv2.cvtColor(np.array(image_pil), cv2.COLOR_RGB2BGR)
    depth_dam2 = _dam2.infer_image(image_bgr)
...
def process_frame(image_pil: Image, depth: torch.Tensor, scale_factor=0.15):
    ...
    """
    disparity = normalize_disp(depth) * scale_factor * _IMAGE_SIZE

    renders = _genstereo_nvs(src_image=image_pil, src_disparity=disparity, ratio=None)
    warped = (renders['warped'] + 1) / 2
    mask = morphological_opening(renders['mask'])

    with torch.no_grad():
        fusion_image = _fusion_model(renders['synthesized'].float(), warped.float(), mask.float())
...
def process_video(video_path, depth_video_path, output_dir, scale_factor=0.15, skip_frames=1, use_ffmpeg=True, crf=23, preset='medium', fast_mode=False):
    ...
    Args:
        ...
        fast_mode: Use SD v1.5 (512x512) for faster processing
    """
    # Load models first
    image_size = load_models(fast_mode)
    ...
    sd_version = "v1.5" if fast_mode else "v2.1"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--fast", action='store_true', help="Use SD v1.5 (512x512) - 3-4x faster processing, slightly lower quality")

    args = parser.parse_args()

    if args.fast:
        print("🚀 FAST MODE: Using SD v1.5 (512x512) - significantly faster")
    else:
        print("🎯 QUALITY MODE: Using SD v2.1 (768x768)")
        print("   Use --fast for 3-4x speedup")
